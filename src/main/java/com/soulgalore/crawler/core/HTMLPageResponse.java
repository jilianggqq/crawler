...
public class HTMLPageResponse {
	 ...
	 * @param theFetchTime
	 *            the time it took to fetch the response
	 */
	public HTMLPageResponse(CrawlerURL pageUrl, int theResponseCode, Map<String, String> theHeaders, String theBody, String theEncoding, long theSize,
			String theResponseType, long theFetchTime) {
		...
		if (!pageUrl.isWrongSyntax()) {
			final String baseUri = pageUrl.getUri().getScheme() + "://" + pageUrl.getUri().getHost()
					+ ((pageUrl.getUri().getPort() != NO_HTTP_PORT) ? ":" + pageUrl.getUri().getPort() : "")
					+ ((pageUrl.getUri().getPath().contains(".")) ? pageUrl.getUri().getPath()
							: pageUrl.getUri().getPath() + (pageUrl.getUri().getPath().endsWith("/") ? "" : "/"));
	...
	public String toString() {
		...
		return this.getClass().getSimpleName() + "url:" + getUrl() + "responseCode:" + getResponseCode() + "encoding:" + encoding + " type:"
				+ responseType;
	}
